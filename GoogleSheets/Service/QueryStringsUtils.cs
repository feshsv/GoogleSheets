...
{
    internal static class QueryStringsUtils
    {
        /// <summary>
        ...
        private static string GetCreateTableQueryString(IEnumerable<object> titles)
        {
            var createTableQuery =
                titles.Aggregate("CREATE TABLE DataFromGoogleSheet(", (current, title) => current + ("[" + title + "]" + " NVARCHAR(MAX), "));
        ...
        private static List<string> GetInsertDataQueryString(IList<object> titles, IList<IList<object>> sheetValues)
        {
            ...
            var count = 0;
            var createTableQuery = "INSERT INTO DataFromGoogleSheet(";
            ...
            while (sheetValues.Count > 0)
            {
                ...
                createTableQuery = (from string onecell in sheetValues[0] select onecell.Replace("'", " ")).
                    Aggregate(createTableQuery, (current, temponecell) => current + ("'" + temponecell + "', "));
